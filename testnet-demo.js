...
async function main() {
  ...
  try {
    // Direct API call to 1inch - trying different endpoints
    const endpoints = [
      `https://api.1inch.dev/orderbook/v3.0/${11155111}/order`,
      `https://api.1inch.dev/orderbook/${11155111}/v3.0/order`,
      `https://api.1inch.dev/orderbook/${11155111}/api/v3.0/order`,
      `https://limit-orders.1inch.io/v3.0/${11155111}/order`,
    ];
...
`;
    ...
    if (!success) {
      console.log("\n❌ Could not find the correct API endpoint");
      console.log(
        "The order was created and signed successfully, but submission failed."
      );
    }
  } catch (error) {
